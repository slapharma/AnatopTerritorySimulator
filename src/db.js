...
// EventEmitter error and it takes the whole Node process down. The pool
// removes the broken client and opens a new one on the next query on its own;
// this handler only stops that from being fatal.
pool.on('error', (err) => console.error('[db] pool error (connection dropped, pool recovers automatically):', err.message));
