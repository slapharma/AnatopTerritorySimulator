...
10. **Clinical `cv.md` verification pass.** The file's own header flags that the CRO
    references were not individually verified. Either verify them or soften the claims.
11. **Add `sql/schema.sql`** (audit item 40). Twelve tables, three of them created live by
    a session that has since ended, exist only in Supabase. There is no way to stand this
    project up from the repo.
